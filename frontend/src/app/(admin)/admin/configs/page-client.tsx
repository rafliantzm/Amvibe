...
import { 
  Key, CheckCircle2, XCircle, Loader2, Save, 
  RefreshCw, ChevronDown, Sparkles, ShieldCheck, AlertTriangle,
  Eye, EyeOff, Zap, Activity, Info, Terminal
} from 'lucide-react'
import type { AdminAiConfigPayload, ConfigData, Model } from '@/lib/admin/ai-config'

interface AiConfigPageClientProps {
  initialData: AdminAiConfigPayload
}

export default function AiConfigPageClient({ initialData }: AiConfigPageClientProps) {
  const [config, setConfig] = useState<ConfigData | null>(initialData.config)
  const [models, setModels] = useState<Model[]>(initialData.availableModels)
  const [activeModel, setActiveModel] = useState<string>(initialData.activeModel)
  const [usingEnvFallback, setUsingEnvFallback] = useState(initialData.usingEnvFallback)
  const [dbError, setDbError] = useState<string | null>(initialData.dbError)
  const [apiKey, setApiKey] = useState('')
  const [selectedModel, setSelectedModel] = useState(initialData.activeModel)
  ...
  const applyConfigData = useCallback((data: AdminAiConfigPayload) => {
  ...
  const fetchConfig = useCallback(async () => {
    ...
    try {
      ...
      console.error(error)
      setStatus({ type: 'error', message: `FETCH_ERR: ${getErrorMessage(error)}` })
    }
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      setStatus({ type: 'error', message: `SYS_ERR: ${getErrorMessage(error)}` })
    }
  }
  ...
  const isLoading = isRefreshing && !config && models.length === 0
    ...
    >
      ...
      >
        ...
        <div className="p-6 relative z-20">
          ...
          {isRefreshing && !isLoading && (
            <div className="mt-5 inline-flex items-center space-x-2 text-blue-400 font-mono text-[10px] uppercase tracking-[0.2em]">
              <Loader2 size={12} className="animate-spin" />
              <span>Synchronizing config snapshot...</span>
            </div>
          )}
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="p-6 space-y-8 relative z-20">
          ...
          {/* Action Buttons */}
          <div className="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-4 pt-4 border-t border-white/[0.05]">
            <button
              onClick={handleTest}
              disabled={isBusy}
              className="flex items-center justify-center space-x-2 w-full sm:w-48 py-3 bg-[#050505] border border-[#581c87]/50 text-purple-400 hover:bg-purple-900/20 hover:border-purple-400 disabled:opacity-50 disabled:cursor-not-allowed transition-all text-[11px] font-mono uppercase tracking-[0.2em]"
            >
              <ShieldCheck size={14} />
              <span>TEST LINK</span>
            </button>
      ...
      >
        ...
        <ul className="space-y-3 text-[11px] font-mono text-[#888] tracking-widest relative z-20">
          ...
          <li className="flex items-start space-x-3">
            ...
            <span>Fallback sequence: Database to <code className="text-blue-400">.env</code> keys.</span>
          </li>
          <li className="flex items-start space-x-3">
