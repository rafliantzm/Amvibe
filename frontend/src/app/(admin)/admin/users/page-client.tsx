...
import {
  ...
  Eye, X, Layers, UserX, UserCheck, Info,
} from 'lucide-react'
import { GlitchText } from '@/components/ui/GlitchText'
import type { AdminUsersPayload, AdminUsersStats, UserRow } from '@/lib/admin/users'
...
// Legacy helper kept temporarily while this page is being cleaned up incrementally.
...
function ProviderBadgeDisplay({ provider }: { provider: string }) {
  if (provider === 'google') {
    return (
      <span className="inline-flex items-center gap-1 text-[9px] font-mono uppercase tracking-widest bg-white/5 border border-white/10 px-1.5 py-0.5 text-[#888]">
        <Mail size={9} />Google
...
export default function UserManagementPageClient({ initialData }: UserManagementPageClientProps) {
  ...
  const [stats, setStats] = useState<AdminUsersStats>(initialData.stats)
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      showToast('error', `FETCH_ERR: ${getErrorMessage(error)}`)
    }
  ...
  const filtered = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      if (sortBy === 'projectCount') {
        ...
        av = a[sortBy] ? new Date(a[sortBy] as string).getTime() : 0
        bv = b[sortBy] ? new Date(b[sortBy] as string).getTime() : 0
      }

      return sortDir === 'desc' ? bv - av : av - bv
    })
    return result
  }, [users, deferredSearch, sortBy, sortDir])
  ...
  const handleBanToggle = async (user: UserRow) => {
    ...
    try {
      ...
      showToast('error', `${action.toUpperCase()}_ERR: ${getErrorMessage(error)}`)
    }
  ...
  const handleDelete = async (user: UserRow) => {
    ...
    try {
      ...
      showToast('error', `DELETE_ERR: ${getErrorMessage(error)}`)
    }
    setActionLoading(null)
  }
  ...
  return (
    ...
    >
        ...
        {[
          ...
          <div key={s.label} className={`p-5 bg-[#020202] border relative overflow-hidden ${
            ...
            <h3 className={`text-5xl font-light tracking-tighter ${
              ...
              <GlitchText text={s.value.toString()} delay={index * 120} />
            </h3>
          </div>
        ))}
      </motion.div>
      ...
      <motion.div variants={itemVariants} className="bg-[#020202] border border-[#1e3a8a]/50 shadow-[inset_0_0_20px_rgba(30,58,138,0.1)] relative">
        ...
        {isRefreshing && (
          <div className="px-6 py-2 border-t border-white/[0.04] text-[10px] font-mono uppercase tracking-[0.2em] text-blue-400 flex items-center gap-2">
            <Loader2 size={12} className="animate-spin" />
            <span>Synchronizing registry snapshot...</span>
          </div>
        )}
        <div className="px-6 pt-3 md:hidden">
          <div className="inline-flex items-center gap-2 rounded-full border border-blue-400/15 bg-blue-500/5 px-3 py-1.5 text-[9px] font-mono uppercase tracking-[0.22em] text-blue-300/80">
            <span className="h-1.5 w-1.5 rounded-full bg-blue-400/80" />
            <span>Swipe table sideways for status and actions</span>
          </div>
        </div>
        ...
        <div className="relative z-20">
          <div className="pointer-events-none absolute inset-y-0 right-0 z-30 w-8 bg-gradient-to-l from-[#020202] via-[#020202]/90 to-transparent md:hidden" />
          <div className="app-scroll-surface overflow-x-auto pb-2">
            <table className="min-w-[860px] w-full text-[11px] font-mono">
            <thead>
              ...
              {isRefreshing && filtered.length === 0 ? (
                ...
                filtered.map((user, i) => (
                  ...
                  >
                    ...
                    {/* Provider */}
                    <td className="px-5 py-4"><ProviderBadgeDisplay provider={user.provider} /></td>
                    {/* Joined */}
                    <td className="px-5 py-4 text-[#888] text-[10px] tracking-widest whitespace-nowrap">{displayDate(user.createdAt)}</td>
                    {/* Last Seen */}
                    <td className="px-5 py-4 text-[#888] text-[10px] tracking-widest whitespace-nowrap">{displayDate(user.lastSignIn)}</td>
                    {/* Projects */}
            ...
            >
              ...
              <div className="p-6 pb-24 space-y-6">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <p className="text-[#ededed] font-medium tracking-tight text-base">{selectedUser.name ?? 'N/A'}</p>
                    <p className="text-[#555] font-mono text-[11px] tracking-widest mt-0.5">{selectedUser.email}</p>
                    <div className="mt-2 flex items-center space-x-2">
                      <ProviderBadgeDisplay provider={selectedUser.provider} />
                      {selectedUser.banned
                ...
                <div className="space-y-3 bg-[#020202] border border-white/[0.05] p-4">
                  {[
                    ...
                    { label: 'Joined', value: displayDate(selectedUser.createdAt), mono: true },
                    { label: 'Last Sign-in', value: displayDate(selectedUser.lastSignIn), mono: true },
                  ].map(row => (
